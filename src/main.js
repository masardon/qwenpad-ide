...
async function onDeviceReady() {
	...
	// Global styles for enhanced UI/UX
const globalStyles = `
  ...
  @media (prefers-color-scheme: dark) {
    ...
    .enhanced-input {
      ...
      border-color: var(--bdr-color-dark, #444);
    }
...
document.head.appendChild(styleElement);
	...
	try {
		...
		setTimeout(async () => {
			...
			// Initialize QwenPad IDE enhanced features
			try {
				await DependencyManager; // This will initialize the singleton
				window.log("info", "QwenPad IDE dependency manager initialized");
			} catch (error) {
				window.log("error", "Failed to initialize QwenPad IDE features:");
				window.log("error", error);
