...
export default class CloudProviderUI {
      ...
      children: [
        ...
        tag('div', {
          ...
          children: [
            ...
            tag('button', {
              textContent: 'Edit',
              onclick: () => this._editConnection(connection.id),
              style: {
                padding: '4px 8px',
                border: '1px solid var(--bdr-color)',
                borderRadius: '4px',
  ...
  _editConnection(connectionId) {
    const connection = this.connections.find(c => c.id === connectionId);
    if (connection) {
      this._showConnectionForm(connection);
    }
  }
  ...
  _deleteConnection(connectionId) {
    ...
    try {
      ...
      toast(`Failed to delete connection: ${error.message}`, 'error');
    }
  }
  ...
  _testConnection(connectionId) {
    ...
    this.manager.testConnection(connection)
      .then(() => {
  ...
  _refresh() {
    try {
      ...
      toast(`Failed to refresh connections: ${error.message}`, 'error');
  ...
  _installCLI() {
    ...
    this.manager.installCLI(providerId)
      .then(() => {
      ...
      .catch(error => {
        ...
        toast(`Failed to install ${provider.name} CLI: ${error.message}`, 'error');
        this._updateStatusBar(`Failed to install ${provider.name} CLI`, 'error');
      });
  }
  ...
  _updateCLI() {
    ...
    this.manager.updateCLI(providerId)
      .then(() => {
      ...
      .catch(error => {
        ...
        toast(`Failed to update ${provider.name} CLI: ${error.message}`, 'error');
        this._updateStatusBar(`Failed to update ${provider.name} CLI`, 'error');
      });
  }
  ...
  _showConnectionForm(connection = null) {
    ...
    // Create form
    const $form = this._createConnectionForm(connection);
    
    $modal.appendChild($form);
    document.body.appendChild($modal);
  }

  _createConnectionForm(connection = null) {
    const isEdit = !!connection;
    const title = isEdit ? 'Edit Connection' : 'New Connection';
    
    // Get provider for this connection or first available
    const providerId = connection?.provider || this.providers[0]?.id;
    const provider = this.providers.find(p => p.id === providerId);
    
    return tag('div', {
      className: 'cloud-connection-form',
      style: {
        backgroundColor: 'var(--bg-color)',
        borderRadius: '8px',
        padding: '24px',
      ...
      children: [
        tag('div', {
          ...
          children: [
            ...
            tag('button', {
              innerHTML: '&times;',
              onclick: () => document.body.removeChild(document.querySelector('.cloud-connection-modal')),
              style: {
                background: 'none',
                border: 'none',
                fontSize: '1.5em',
  ...
  async _saveConnection(existingId = null) {
    try {
      ...
      toast(`Error saving connection: ${error.message}`, 'error');
